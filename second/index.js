...
exports.bitcoinInfo = (req, res) => {
  ...
  console.log(`bitcoinInfoAction Request headers:
${JSON.stringify(req.headers)}`);
  ...
  const priceHandler = app => {
    ...
    request(EXT_BITCOIN_API_URL + EXT_PRICE, (error, response, body) => {
      ...
      console.log(`priceHandler response: ${JSON.stringify(response)} Body:
${body} | Error: ${error}`);
      // 返事のメッセージ文字列を組み立てます。
  ...
  const totalHandler = app => {
    ...
    request(EXT_BITCOIN_API_URL + EXT_TOTAL, (error, response, body) => {
      // Bitcoin APIの結果をログ出力します。
      console.log(`totalHandler response: ${JSON.stringify(response)} Body:
${body} | Error: ${error}`);
      // 総数の桁が大きいので、億の単位にします。
...
${body} | error: ${error}`);
      ...
      request(
        ...
        (error, response, body) => {
          // 通貨レートAPIの結果をログ出力します。
          console.log(`Fixer API response: ${JSON.stringify(response)} body:
${body} | error: ${error}`);
